...
export const logIntro = () => {
  try {
    ...
    console.log(
      `%cUI to Code${version ? ` v${version}` : ""}%c\nhttps://ui-to-code.dev`,
    ...
    if (navigator.onLine && version && !isExtensionContext()) {
      fetch(
        `https://www.ui-to-code.dev/api/version?source=browser&t=${Date.now()}`,
        {
        ...
        .then((latestVersion) => {
          if (latestVersion && latestVersion !== version) {
            console.warn(
              `[UI to Code] v${version} is outdated (latest: v${latestVersion})`,
            );
          }
        })
        .catch(() => null);
