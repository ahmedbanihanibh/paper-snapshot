...
// click "after" segment at x365 w39 y~107
await click(365+20, 118); await sleep(1000);
console.log('date operator menu:', await evalJs(`(()=>{
  const fixed=[...document.querySelectorAll('*')].filter(e=>getComputedStyle(e).position==='fixed');
  const cand=fixed.filter(e=>{const r=e.getBoundingClientRect();return r.width>80&&r.width<400&&r.height>40&&r.height<500});
  const m=cand[cand.length-1]; if(!m)return 'none';
  const b=m.getBoundingClientRect();
  const rows=[...m.querySelectorAll('div,button,[role]')].filter(x=>{const r=x.getBoundingClientRect();return r.height>20&&r.height<42&&r.width>60}).map(x=>x.textContent.replace(/\\s+/g,' ').slice(0,40));
...
if (clr) { const p=JSON.parse(clr); await click(p.x,p.y); await sleep(600); }
