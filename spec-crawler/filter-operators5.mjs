...
const overlay=async()=>await evalJs(`(()=>{
  ...
  const cand=fixed.filter(e=>{const r=e.getBoundingClientRect();return r.width>100&&r.width<500&&r.height>40&&r.height<600});
...
const chipSeg=async(txt)=>JSON.parse(await evalJs(`(()=>{
  const out=[];
  for(const e of document.querySelectorAll('*')){
    const r=e.getBoundingClientRect();
    if(r.y>95&&r.y<135&&r.x>240&&r.x<800&&r.height>18&&r.height<28&&e.children.length<=3){
      const t=e.textContent.replace(/\\s+/g,' ').trim();
      if(t)out.push({t,x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)});
    }
  }
  return JSON.stringify(out);
})()`)).filter(p=>p.t===txt)[0];

// 1) click the VALUE segment ("Todo") — value picker; add In Progress
const val = await chipSeg('Todo');
if (val) {
  await click(val.x+val.w/2, val.y+val.h/2); await sleep(900);
  console.log('value picker:', (await overlay()).slice(0,300));
  // type + select In Progress via keyboard
  await send('Input.insertText',{text:'in prog'}); await sleep(600);
  await key('Enter','Enter',13); await sleep(600);
  await key('Escape','Escape',27); await sleep(600);
  console.log('path after esc:', await evalJs('location.pathname'));
  // re-read chip
  const band = await evalJs(`(()=>{
    const out=[]; for(const e of document.querySelectorAll('*')){const r=e.getBoundingClientRect();
      if(r.y>95&&r.y<135&&r.x>240&&r.x<900&&r.height>18&&r.height<28&&e.children.length<=4){const t=e.textContent.replace(/\\s+/g,' ').trim();if(t&&t.length<50)out.push({t,x:Math.round(r.x),w:Math.round(r.width)});}}
    return JSON.stringify(out);
  })()`);
  console.log('chip after 2 values:', band);
  // open operator menu now
  const parts = JSON.parse(band);
  const op = parts.find(p=>/^is/.test(p.t)&&p.w<130);
  if (op) {
    const seg = await chipSeg(op.t);
    if (seg) { await click(seg.x+seg.w/2, seg.y+seg.h/2); await sleep(900);
      console.log('operator menu multi:', await overlay()); await key('Escape','Escape',27); await sleep(400); }
  }
}
