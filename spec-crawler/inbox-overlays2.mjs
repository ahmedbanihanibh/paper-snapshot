...
const overlay=async()=>await evalJs(`(()=>{
  ...
  const cand=fixed.filter(e=>{const r=e.getBoundingClientRect();return r.width>150&&r.width<450&&r.height>60&&r.height<600});
  const m=cand[cand.length-1]; if(!m)return 'none';
  const b=m.getBoundingClientRect();
  const rows=[...m.querySelectorAll('div,button,[role]')].filter(x=>{const r=x.getBoundingClientRect();return r.height>20&&r.height<44&&r.width>120}).map(x=>x.textContent.replace(/\\s+/g,' ').slice(0,48));
...
for (const [x,y,label] of [[317,28,'Notification actions'],[586,28,'Add filter (inbox)'],[620,28,'Display options (inbox)']]) {
  await click(x,y); await sleep(900);
  console.log(label, ':', await overlay());
