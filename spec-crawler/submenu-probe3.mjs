...
for (const t of [300, 800, 1500]) {
  await sleep(t===300?300:t-((t===800)?300:800));
  const dump = await evalJs(`(()=>{
    const providers=[...document.querySelectorAll('div[class*="theme-provider"]')];
    return JSON.stringify(providers.map(p=>{
      const r=p.getBoundingClientRect();
      const roles=[...p.querySelectorAll('[role]')].map(e=>e.getAttribute('role'));
      return {w:Math.round(r.width),h:Math.round(r.height),x:Math.round(r.x),y:Math.round(r.y),roles:[...new Set(roles)],txt:p.textContent.replace(/\\s+/g,' ').slice(0,120)};
    }).filter(p=>p.w>0));
  })()`);
  console.log('t='+t+':', dump);
}
ws.close();
